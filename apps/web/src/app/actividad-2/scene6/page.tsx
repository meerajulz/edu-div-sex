...
export default function Scene6Page() {
  ...
  const playSound = () => {
    try { playGameAudio('/audio/button/Bright.mp3', 0.7, 'Button Click Sound'); } catch (error) { console.warn(error); }
  };
  ...
  const handleButtonClick = () => {
    ...
    setIsAnimating(true);
    playSound();
    setTimeout(() => { setIsAnimating(false); setShowJuegoCinco(true); }, 800);
  };
  ...
  const handleGameComplete = () => {
    ...
    setTimeout(() => setShowAlexCongratulations(true), 1000);
  };
  ...
  const handleGoToHome = async () => {
    ...
    setIsAnimating(true);
    playSound();
    await saveProgress('actividad-2', 'scene6', 'completed', 100, {
  ...
  return (
    ...
    >
      ...
      {showJuegoCinco && (
        <div className="fixed inset-0 z-30 bg-cover bg-center" style={{ backgroundImage: "url('/image/actividad_2/bg.png')" }} />
      )}
      ...
      {!showJuegoCinco && !showAlexCongratulations && !showCongratulations && (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          <motion.div animate={isAnimating ? { scale: [1, 1.3, 1], rotate: [0, -360] } : {}} transition={{ duration: 0.8 }}>
            <JugarButton text='TU COFRE DE LA INTIMIDAD' onClick={handleButtonClick} disabled={isAnimating} />
          </motion.div>
        </div>
      )}
      ...
      {showCongratulations && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-3xl font-bold text-white mb-4">¡Excelente!</h2>
            <p className="text-white text-lg mb-6">Has completado la Actividad Intimidad</p>
            <div className="flex flex-col items-center gap-4">
              ...
              {hasWatchedFinalVideo && <VolverAVerButton onClick={handleReplayFinalVideo} />}
            </div>
          </motion.div>
        </motion.div>
      )}
