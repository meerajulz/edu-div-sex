...
import JuegoUnoAventura5 from './JuegoUnoAventura5/JuegoUnoAventura5';

export default function Aventura5Scene1Page() {
  useActivityTracking();
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('av5-scene1-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('av5-scene1-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleGoToHub = async () => {
    ...
    setIsAnimating(true);
    playSound();
    await saveProgress('aventura-5', 'scene1', 'completed', 100, {
      video_watched: true,
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-b from-purple-200 via-pink-100 to-orange-200 z-0" />
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text="LOS DIFERENTES TIPOS DE PAREJA" onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
        <div className="absolute" style={containerStyle}>
          {!videoEnded ? (
            <>
              <OptimizedVideo
                ref={videoRef}
                src="/video/Actividad_5_scene_1.mp4"
      ...
      {showCongratulations && (
        ...
        >
          <motion.div
            className="bg-gradient-to-br from-purple-300 via-pink-400 to-orange-500 p-8 rounded-3xl shadow-2xl max-w-md mx-4 text-center"
