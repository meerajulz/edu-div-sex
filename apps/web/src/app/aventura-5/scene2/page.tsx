...
export default function Aventura5Scene2Page() {
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('av5-scene2-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('av5-scene2-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleGoToHub = async () => {
    ...
    setIsAnimating(true);
    playSound();
    await saveProgress('aventura-5', 'scene2', 'completed', 100, {
      video_watched: true,
      completed_at: new Date().toISOString(),
    });
    setTimeout(() => { setIsAnimating(false); router.push('/aventura-5'); }, 800);
  };
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-b from-orange-200 via-pink-100 to-purple-200 z-0" />
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text="EL RESPETO A LOS DIFERENTES TIPOS DE PAREJA" onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
        <div className="absolute" style={containerStyle}>
          {!videoEnded ? (
            <>
              <OptimizedVideo
                ref={videoRef}
                src="/video/Actividad_5_scene_2.mp4"
                ...
                onLoadedData={() => {
                  if (videoRef.current) videoRef.current.volume = currentVolume;
                }}
                onPlay={async () => {
                  const video = videoRef.current;
                  if (!video) return;
      ...
      {showCongratulations && (
        ...
        >
          <motion.div
            className="bg-gradient-to-br from-orange-300 via-pink-400 to-purple-500 p-8 rounded-3xl shadow-2xl max-w-md mx-4 text-center"
          ...
          >
            ...
            <motion.button
              ...
              className="bg-white text-orange-600 font-bold py-3 px-6 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50"
