...
import { AVENTURA_5_CONFIG } from '../components/ActivityMenu/activityConfig';
...
export default function Aventura5NivelAvanzadoPage() {
  ...
  useEffect(() => {
    const hasSeenIntro = localStorage.getItem('aventura-5-intro-seen') === 'true';
    if (hasSeenIntro) {
      setVideoEnded(true);
      setShowSun(true);
  ...
  const playSound = () => {
    try {
      const audio = new Audio('/audio/advance-aventura5/aventura5-title.mp3');
      audio.volume = 0.7;
  ...
  const handleVideoEnd = () => {
    cleanupAudio();
    stopBackgroundMusic('aventura-5-bg');
    localStorage.setItem('aventura-5-intro-seen', 'true');
    setVideoEnded(true);
    setShowSun(true);
    setTimeout(() => setShowArdilla(true), 100);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      {!videoEnded && (!canPlayVideo || needsInteraction) && (
        <>
          <div className="absolute inset-0 z-50 bg-gradient-to-b from-purple-300 via-pink-300 to-orange-300" />
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center">
            ...
            >
              <JugarButton text="Diversidad sexual" onClick={handleJugarClick} />
            </motion.div>
          </div>
        </>
      )}
      ...
      {isExiting && (
        <div className="fixed inset-0 bg-gradient-to-b from-purple-200 via-pink-200 to-orange-300 z-0" />
      )}
