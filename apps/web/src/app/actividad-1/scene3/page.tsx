...
//const { data: session } = useSession();
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      console.log(`🍎 Scene3 iPhone: Connecting video to Web Audio API...`);
      ...
      console.log(`🍎 Scene3 iPhone: ✅ Video connected to Web Audio`);
    } catch (e) {
      console.error('🍎 Scene3 iPhone: ❌ Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('scene3-video-watched'));
    console.log('📱 Scene3: Device info initialized:', info);
  }, []);
  ...
  useEffect(() => {
    const handleVolumeChange = (event: CustomEvent) => {
      ...
      console.log(`🎵 Scene3: Volume change: ${volume}`);

      // Apply to both main video and scene2 replay video
      const videos = [videoRef.current, document.querySelector('[src="/video/ACTIVIDAD_1_ESCENA_2.mp4"]')] as HTMLVideoElement[];
  ...
  const handleVideoEnd = async () => {
    ...
    localStorage.setItem('scene3-video-watched', 'true');
        ...
        <div className="absolute" style={containerStyle}>
          <OptimizedVideo
            ...
            onLoadedData={() => {
              ...
              if (video) {
                video.volume = currentVolume;
                console.log(`🎬 Scene3: Video loaded, volume: ${currentVolume}`);
              }
            }}
            onPlay={async () => {
              const video = videoRef.current;
              ...
              video.muted = false;
              video.volume = currentVolume;
              console.log(`🎬 Scene3: Video playing, volume: ${currentVolume}`);
