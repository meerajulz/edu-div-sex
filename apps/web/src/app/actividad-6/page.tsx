...
export default function Actividad6Page() {
  ...
  const [deviceInfo] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      console.log(`🎬 Activity6: Device audio info:`, info);
      return info;
    }
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      console.log(`🍎 Activity6 iPhone: Attempting to connect video to Web Audio API...`);
      console.log(`🍎 Activity6 iPhone: Video readyState: ${video.readyState}`);
      console.log(`🍎 Activity6 iPhone: AudioContext state: ${audioContext.state}`);
      ...
      if ((video as HTMLVideoElement & { _webAudioConnected?: boolean })._webAudioConnected) {
        console.log(`🍎 Activity6 iPhone: Video already connected to Web Audio API, skipping`);
        return;
      }

      // Create MediaElementSource from video
      const source = audioContext.createMediaElementSource(video);
      console.log(`🍎 Activity6 iPhone: MediaElementSource created successfully`);
      ...
      if (!sharedGainNode) {
        ...
        console.log(`🍎 Activity6 iPhone: Created shared gainNode (FloatingMenu will use this same one)`);
      } else {
        console.log(`🍎 Activity6 iPhone: Using pre-existing shared gainNode`);
      }

      // Set initial volume
      sharedGainNode.gain.value = currentVolume;
      console.log(`🍎 Activity6 iPhone: Shared gainNode value set to: ${sharedGainNode.gain.value}`);

      // Connect: video -> sharedGainNode -> speakers
      source.connect(sharedGainNode);
      console.log(`🍎 Activity6 iPhone: Video source connected to shared gainNode`);

      sharedGainNode.connect(audioContext.destination);
      console.log(`🍎 Activity6 iPhone: Shared gainNode connected to AudioContext destination`);
      ...
      console.log(`🍎 Activity6 iPhone: Video successfully connected to Web Audio API for volume control`);

    } catch (error) {
      console.error(`🍎 Activity6 iPhone: Failed to connect video to Web Audio API:`, error);
    }
  };
...
useEffect(() => {
  const handleGlobalVolumeChange = (e: CustomEvent) => {
    ...
    console.log(`🎵 Activity6: Received global volume change: ${volume} (iPhone: ${isIPhone}, iPad: ${isIPad})`);
    ...
    if (video && video.readyState > 0) {
      if (isIPhone) {
        ...
        if (sharedGainNode && sharedGainNode.gain) {
          ...
          console.log(`📱 Activity6 iPhone: Volume applied via shared gainNode: ${volume}`);
        }
      } else if (isIPad) {
        // iPad: Direct volume control
        video.volume = volume;
        video.muted = volume === 0;
        console.log(`🔲 Activity6 iPad: Applied volume ${volume} directly (muted: ${volume === 0})`);
      } else {
        // Desktop/Android: Direct video volume
        video.muted = false;
        video.volume = volume;
        console.log(`🖥️ Activity6 Desktop/Android: Applied volume ${volume} directly`);
      }
    }
  };
  ...
  const handleJugarClick = async () => {
    ...
    try {
      await initAudio();
      console.log('🍎 Activity6: Audio initialized for volume control');
    } catch (e) {
      console.warn('Activity6: Audio initialization failed:', e);
    }

    // Start background music using new background music system
    // This will respond to volume changes from FloatingMenu
    playBackgroundMusic('/audio/Softy.mp3', 0.4, 'actividad-6-bg');
      ...
      {!videoEnded && (!canPlayVideo || needsInteraction) && (
        <>
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center">
            ...
            <JugarButton text='Jugar' onClick={handleJugarClick} />
            </motion.div>
          </div>
        </>
      )}
          ...
          {!videoEnded && canPlayVideo ? (
            <video
              ...
              onLoadedMetadata={async () => {
                console.log(`🎬 Activity6: Video metadata loaded`);
                const video = videoRef.current;
                if (!video) return;
                ...
                try {
                  await initAudio();
                  console.log('🍎 Activity6: Audio initialized');
                } catch (e) {
                  console.warn('Activity6: Audio init failed:', e);
                }

                if (isIPhone) {
                  console.log(`📱 Activity6 iPhone: Setting up Web Audio API for volume control`);

                  // Get or create AudioContext for iPhone volume control
                  if (!window.globalAudioContext) {
                    window.globalAudioContext = new (window.AudioContext || window.webkitAudioContext)();
                    console.log(`📱 Activity6 iPhone: AudioContext created`);
                  }
                  ...
                  try {
                    ...
                    console.log(`🔲 Activity6 iPad: Audio context initialized for playGameAudio`);
                  } catch (error) {
                    console.error('🔲 Activity6 iPad: Error setting up audio context:', error);
                  }
                }
              }}
              onCanPlay={() => {
                console.log(`🎬 Activity6: Video can play`);
                const video = videoRef.current;
                if (!video) return;
                ...
                if (isIPhone) {
                  ...
                  console.log(`📱 Activity6 iPhone: Video ready, using Web Audio volume control`);
                } else if (isIPad) {
                  video.volume = currentVolume;
                  video.muted = currentVolume === 0;
                  console.log(`🔲 Activity6 iPad: Video ready, direct volume: ${currentVolume} (muted: ${currentVolume === 0})`);
                } else {
                  video.muted = false;
                  video.volume = currentVolume;
                  console.log(`🖥️ Activity6 Desktop/Android: Video ready, volume: ${currentVolume}`);
                }
              }}
            />
          ) : (
