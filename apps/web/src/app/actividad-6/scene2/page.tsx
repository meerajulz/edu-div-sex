...
//const { data: session } = useSession();
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      console.log(`🍎 Activity6-Scene2 iPhone: Connecting video to Web Audio API...`);
      ...
      console.log(`🍎 Activity6-Scene2 iPhone: ✅ Video connected to Web Audio`);
    } catch (e) {
      console.error('🍎 Activity6-Scene2 iPhone: ❌ Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('a6-scene2-video-watched'));
    console.log('📱 Activity6-Scene2: Device info initialized:', info);
  }, []);
  ...
  useEffect(() => {
    const handleVolumeChange = (event: CustomEvent) => {
      ...
      console.log(`🎵 Activity6-Scene2: Received global volume change: ${volume}`);
      ...
      if (video && video.readyState > 0) {
        ...
        if (isIPhone) {
          ...
          if (window.videoGainNode) {
            ...
            console.log(`📱 Activity6-Scene2 iPhone: Applied volume ${volume} via Web Audio`);
          }
        } else {
          video.muted = false;
          video.volume = volume;
          console.log(`🖥️ Activity6-Scene2 Desktop/Android/iPad: Applied volume ${volume}`);
        }
      }
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('a6-scene2-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleGoToMenu = async () => {
    ...
    setTimeout(() => {
      setIsAnimating(false);
      if (progressSaved) {
        console.log('✅ Actividad6-Scene2: Progress saved successfully');
      } else {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-purple-200 via-violet-100 to-cyan-300 z-0" />
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text='ME DEFIENDO' onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
        <div className="absolute" style={containerStyle}>
          {!videoEnded ? (
            ...
            <OptimizedVideo
              ref={videoRef}
              src="/video/ACTIVIDA_6-ESCENA_2.mp4"
              ...
              onLoadedData={() => {
                ...
                if (video) {
                  ...
                  console.log(`🎬 Activity6-Scene2: Video loaded, volume: ${currentVolume}`);
                }
              }}
              onPlay={async () => {
                const video = videoRef.current;
                ...
                video.muted = false;
                video.volume = currentVolume;
                console.log(`🎬 Activity6-Scene2: Video playing, volume: ${currentVolume}`);
                ...
                try {
                  ...
                  console.error('Activity6-Scene2: Audio setup failed:', error);
                }
              }}
              lazyLoad={true}
              lowPowerMode={true}
