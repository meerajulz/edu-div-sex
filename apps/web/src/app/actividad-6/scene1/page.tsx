...
//const { data: session } = useSession();
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      console.log(`🍎 Activity6-Scene1 iPhone: Connecting video to Web Audio API...`);
      ...
      console.log(`🍎 Activity6-Scene1 iPhone: ✅ Video connected to Web Audio`);
    } catch (e) {
      console.error('🍎 Activity6-Scene1 iPhone: ❌ Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('a6-scene1-video-watched'));
    console.log('📱 Activity6-Scene1: Device info initialized:', info);
  }, []);
  ...
  useEffect(() => {
    const handleVolumeChange = (event: CustomEvent) => {
      ...
      console.log(`🎵 Activity6-Scene1: Received global volume change: ${volume}`);
      ...
      if (video && video.readyState > 0) {
        ...
        if (isIPhone) {
          ...
          if (window.videoGainNode) {
            ...
            console.log(`📱 Activity6-Scene1 iPhone: Applied volume ${volume} via Web Audio`);
          }
        } else {
          video.muted = false;
          video.volume = volume;
          console.log(`🖥️ Activity6-Scene1 Desktop/Android/iPad: Applied volume ${volume}`);
        }
      }
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('a6-scene1-video-watched', 'true');
    setVideoEnded(true);
    ...
    >
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text='MIS PARTES PRIVADAS' onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
            ...
            <OptimizedVideo
              ...
              onLoadedData={() => {
                ...
                if (video) {
                  ...
                  console.log(`🎬 Activity6-Scene1: Video loaded, volume: ${currentVolume}`);
                }
              }}
              onPlay={async () => {
                const video = videoRef.current;
                ...
                video.muted = false;
                video.volume = currentVolume;
                console.log(`🎬 Activity6-Scene1: Video playing, volume: ${currentVolume}`);
                ...
                try {
                  ...
                  console.error('Activity6-Scene1: Audio setup failed:', error);
                }
              }}
              lazyLoad={true}
              lowPowerMode={true}
