...
//const { data: session } = useSession();
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      console.log(`🍎 Activity4-Scene1 iPhone: Connecting video to Web Audio API...`);
      ...
      console.log(`🍎 Activity4-Scene1 iPhone: ✅ Video connected to Web Audio`);
    } catch (e) {
      console.error('🍎 Activity4-Scene1 iPhone: ❌ Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    const handleVolumeChange = (event: CustomEvent) => {
      ...
      console.log(`🎵 Activity4-Scene1: Received global volume change: ${volume}`);
      ...
      if (video && video.readyState > 0) {
        ...
        if (isIPhone) {
          ...
          if (window.videoGainNode) {
            ...
            console.log(`📱 Activity4-Scene1 iPhone: Applied volume ${volume} via Web Audio`);
          }
        } else {
          video.muted = false;
          video.volume = volume;
          console.log(`🖥️ Activity4-Scene1 Desktop/Android/iPad: Applied volume ${volume}`);
        }
      }
  ...
  const playSound = () => {
    playGameAudio('/audio/button/Bright.mp3', 0.7, 'Scene1 Button Sound');
  };
