// Game Logic Hooks for JuegoDosActividad3Male

import { useState, useRef, useCallback } from 'react';
import { GAME_CONFIG_MALE, SituationAttempt, GameSession } from './config-male';
import { createGameAudio } from '../../../utils/gameAudio';

export const useGameState = () => {
  const gameConfig = GAME_CONFIG_MALE;
  ...
  const playAudio = useCallback((audioPath: string) => {
    return new Promise<void>((resolve, reject) => {
      try {
        ...
        const audio = createGameAudio(audioPath, 1.0, 'JuegoDosActividad3Male');
