...
import { useGameState, useGameSession, useGameTracking, useAudioManager } from './hooks-female';
...
interface JuegoDosActividad3FemaleProps {
  isVisible: boolean;
  onClose: () => void;
  onGameComplete: () => void;
}

const JuegoDosActividad3Female: React.FC<JuegoDosActividad3FemaleProps> = ({
  isVisible,
  onClose,
  onGameComplete
}) => {
  ...
  useEffect(() => {
    if (isVisible) {
      console.log('🎮 Game modal opened (Female version), starting game...');
      resetGame();
      startSession();
      startTitlePhase();
    } else {
  ...
  const startTitlePhase = useCallback(async () => {
    console.log('🎵 Starting title phase (Female)...');
    setGamePhase('title');
  ...
  const startSituationPhase = useCallback(async (situationIndex?: number) => {
    ...
    console.log(`🎮 Starting situation ${situationIdx + 1} (Female):`, situation.title);
  ...
  const handleAnswerSelect = useCallback(async (answer: 'YES' | 'NO') => {
    ...
    console.log(`🎮 Answer selected: ${answer} for situation ${currentSituation + 1} (Female)`);
  ...
  const handleFeedbackComplete = useCallback(() => {
    ...
    if (isCorrect) {
      ...
      if (currentSituation >= gameConfig.situations.length - 1) {
        // Game complete
        console.log('🎉 Game completed (Female version)!');
  ...
  const handleGameComplete = useCallback(() => {
    console.log('🎮 Game completion sequence finished (Female version)');

    try {
      playGameAudio('/audio/button/Bright.mp3', 0.7, 'JuegoDosActividad3Female completion');
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 backdrop-blur-sm flex items-center justify-center pointer-events-auto p-2 sm:p-4">
      ...
      <div
        className="relative w-full h-full max-w-[1200px] max-h-[750px] rounded-xl shadow-xl pointer-events-auto overflow-hidden bg-gradient-to-br from-pink-400 to-purple-600"
        style={{
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-white">
              ...
              <div className="text-2xl font-bold mb-4">
                ¿Cuándo puedes tener ese gustito en la vulva tan, tan fuerte?
              </div>
...
export default JuegoDosActividad3Female;
