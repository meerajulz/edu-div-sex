...
export default function Actividad3Scene1_2Page() {
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      console.log(`🍎 Scene1-2 iPhone: Attempting to connect video to Web Audio API...`);
      console.log(`🍎 Scene1-2 iPhone: Video readyState: ${video.readyState}`);
      ...
      if ((video as HTMLVideoElement & { _webAudioConnected?: boolean })._webAudioConnected) {
        console.log(`🍎 Scene1-2 iPhone: Video already connected to Web Audio API`);
        return;
      }

      // Create MediaElementSource from video
      const source = audioContext.createMediaElementSource(video);
      console.log(`🍎 Scene1-2 iPhone: Created MediaElementSource`);

      // Get or create shared gain node from FloatingMenu
      let sharedGainNode = window.sharedGainNode;
      if (!sharedGainNode) {
        console.log(`🍎 Scene1-2 iPhone: Creating new shared gain node`);
      ...
      console.log(`🍎 Scene1-2 iPhone: Connected video to shared gain node`);
      console.log(`🍎 Scene1-2 iPhone: Gain value: ${sharedGainNode.gain.value}`);
      ...
      console.log(`🍎 Scene1-2 iPhone: ✅ Successfully connected video to Web Audio API`);
      console.log(`🍎 Scene1-2 iPhone: videoGainNode stored: ${!!window.videoGainNode}`);

    } catch {
      console.error('🍎 Scene1-2 iPhone: ❌ FAILED to connect video to Web Audio API');
    }
  };
  ...
  useEffect(() => {
    ...
    setDeviceInfo(info);
    console.log('📱 Scene1-2: Device info initialized:', info);
  }, []);
  ...
  useEffect(() => {
    const handleVolumeChange = (event: CustomEvent) => {
      ...
      console.log(`🎵 Scene1-2: Received global volume change: ${volume} (deviceInfo.isIOS: ${deviceInfo.isIOS})`);
        ...
        if (video && video.readyState > 0) {
          ...
          if (isIPhone) {
            ...
            if (window.videoGainNode) {
              ...
              console.log(`📱 Scene1-2 iPhone: Applied volume ${volume} via Web Audio videoGainNode to video ${index + 1}`);
            } else {
              console.warn(`📱 Scene1-2 iPhone: videoGainNode not available for volume ${volume}`);
            }
          } else {
            // Desktop/Android/iPad: Direct video volume
            video.muted = false;
            video.volume = volume;
            console.log(`🖥️ Scene1-2 Desktop/Android/iPad: Applied volume ${volume} directly to video ${index + 1}`);
          }
        } else {
          console.log(`🎬 Scene1-2: Video ${index + 1} not ready (readyState: ${video?.readyState}), setting volume for later`);
        }
      });
    ...
    >
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text='EL ORGASMO FEMENINO: CLÍTORIS Y PUNTO G' onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
