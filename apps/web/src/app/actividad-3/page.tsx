...
useEffect(() => {
  const handleGlobalVolumeChange = (e: CustomEvent) => {
    ...
    console.log(`🎵 Activity 3: Received global volume change: ${newVolume}`);
    setCurrentVolume(newVolume);

    // Apply volume to video element for iPhone compatibility
    if (deviceInfo.isIOS && videoRef.current) {
      console.log(`🍎 Activity3 iPhone: Applying volume ${newVolume} to video element`);
      ...
      if (sharedGainNode && sharedGainNode.gain) {
        ...
        console.log(`🍎 Activity3 iPhone: Volume applied via shared gainNode: ${newVolume}`);
      }

      // Also apply to video element as fallback
      videoRef.current.volume = newVolume;
      console.log(`🍎 Activity3 iPhone: Volume applied to video element: ${newVolume}`);
    }
  };
            ...
            <video
              ...
              onCanPlay={() => {
                ...
                if (deviceInfo.isIOS && videoRef.current) {
                  ...
                  console.log(`🍎 Activity3 iPhone: Set initial video volume to ${currentVolume}`);
                }
              }}
            />
          ) : (
