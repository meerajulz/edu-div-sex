...
export default function Aventura1Juego1Page() {
  ...
  const handleNext = async () => {
    ...
    await saveProgress('aventura-1', 'juego1', 'completed', 100, {
      completed_at: new Date().toISOString(),
    });

    router.push('/aventura-1/scene4');
  };

  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-green-500 via-emerald-400 to-teal-300">
      ...
      <div className="text-center text-white px-8 max-w-lg">
        <p className="text-sm font-semibold uppercase tracking-widest mb-2 opacity-70">
          Aventura 1 &mdash; Juego 1
        </p>
        <h1 className="text-4xl font-bold mb-4">¿Qué ha cambiado?</h1>
        <p className="text-lg mb-3 opacity-80">
          Pon a prueba lo que has aprendido sobre los cambios en la pubertad.
        </p>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <button
            onClick={handleNext}
            disabled={isSaving}
            className="bg-white text-green-600 font-bold px-6 py-3 rounded-full shadow-lg hover:bg-green-50 transition-colors disabled:opacity-60"
          >
