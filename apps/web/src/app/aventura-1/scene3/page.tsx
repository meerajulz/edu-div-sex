...
export default function Aventura1Scene3Page() {
  ...
  const handleNext = async () => {
    ...
    await saveProgress('aventura-1', 'scene3', 'completed', 100, {
      completed_at: new Date().toISOString(),
    });

    router.push('/aventura-1/juego1');
  };

  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-teal-500 via-blue-400 to-indigo-400">
      ...
      <div className="text-center text-white px-8 max-w-lg">
        <p className="text-sm font-semibold uppercase tracking-widest mb-2 opacity-70">
          Aventura 1 &mdash; Escena 3
        </p>
        <h1 className="text-4xl font-bold mb-4">Erección</h1>
        <p className="text-lg mb-3 opacity-80">
          Conoce más sobre los cambios que experimenta el cuerpo durante la pubertad.
        </p>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <button
            onClick={handleNext}
            disabled={isSaving}
            className="bg-white text-teal-600 font-bold px-6 py-3 rounded-full shadow-lg hover:bg-teal-50 transition-colors disabled:opacity-60"
          >
