...
export default function Aventura1Juego3Page() {
  ...
  const handleViewContent = () => {
    ...
    saveProgress('aventura-1', 'juego3', 'completed', 100, {
  ...
  const handleNext = async () => {
    ...
    await saveProgress('aventura-1', 'juego3', 'completed', 100, {
      completed_at: new Date().toISOString(),
    });

    router.push('/aventura-1/scene7');
  };

  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-rose-500 via-pink-400 to-purple-400">
      ...
      <div className="text-center text-white px-8 max-w-lg">
        <p className="text-sm font-semibold uppercase tracking-widest mb-2 opacity-70">
          Aventura 1 &mdash; Juego 3
        </p>
        <h1 className="text-4xl font-bold mb-4">Higiene menstrual</h1>
        <p className="text-lg mb-3 opacity-80">
          Practica lo aprendido sobre la higiene durante la menstruación.
        </p>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <button
            onClick={handleNext}
            disabled={isSaving}
            className="bg-white text-rose-600 font-bold px-6 py-3 rounded-full shadow-lg hover:bg-rose-50 transition-colors disabled:opacity-60"
          >
