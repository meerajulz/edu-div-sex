...
export default function Aventura1Juego2Page() {
  ...
  const handleViewContent = () => {
    ...
    localStorage.setItem('aventura-1-return-to', '/aventura-1/scene5');
  ...
  const handleNext = async () => {
    ...
    await saveProgress('aventura-1', 'juego2', 'completed', 100, {
      completed_at: new Date().toISOString(),
    });

    router.push('/aventura-1/scene5');
  };

  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-sky-500 via-blue-400 to-indigo-400">
      ...
      <div className="text-center text-white px-8 max-w-lg">
        <p className="text-sm font-semibold uppercase tracking-widest mb-2 opacity-70">
          Aventura 1 &mdash; Juego 2
        </p>
        <h1 className="text-4xl font-bold mb-4">Ayuda en la higiene</h1>
        <p className="text-lg mb-3 opacity-80">
          Practica los hábitos de higiene con este juego interactivo.
        </p>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <button
            onClick={handleNext}
            disabled={isSaving}
            className="bg-white text-sky-600 font-bold px-6 py-3 rounded-full shadow-lg hover:bg-sky-50 transition-colors disabled:opacity-60"
          >
