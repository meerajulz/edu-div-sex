...
import JuegoTresAventura6 from './JuegoTresAventura6/JuegoTresAventura6';

export default function Aventura6Scene3Page() {
  useActivityTracking();
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('av6-scene3-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('av6-scene3-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleGoToHub = async () => {
    ...
    setIsAnimating(true);
    playSound();
    await saveProgress('aventura-6', 'scene3', 'completed', 100, {
      video_watched: true,
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-b from-indigo-200 via-violet-100 to-purple-200 z-0" />
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text="¿CÓMO CORTAR CON TU PAREJA?" onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
        <div className="absolute" style={containerStyle}>
          {!videoEnded ? (
            <>
              <OptimizedVideo
                ref={videoRef}
                src="/video/avanzado/Actividad_6_scene_3.mp4"
      ...
      {showCongratulations && (
        ...
        >
          <motion.div
            className="bg-gradient-to-br from-indigo-300 via-violet-400 to-purple-500 p-8 rounded-3xl shadow-2xl max-w-md mx-4 text-center"
          ...
          >
            ...
            <motion.button
              ...
              className="bg-white text-indigo-600 font-bold py-3 px-6 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50"
