...
export default function Aventura6Scene2Page() {
  useActivityTracking();
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('av6-scene2-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('av6-scene2-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleGoToHub = async () => {
    ...
    setIsAnimating(true);
    playSound();
    await saveProgress('aventura-6', 'scene2', 'completed', 100, {
      video_watched: true,
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-b from-blue-200 via-sky-100 to-indigo-200 z-0" />
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text="¿PUEDO TENER DOS PAREJAS?" onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
        <div className="absolute" style={containerStyle}>
          {!videoEnded ? (
            <>
              <OptimizedVideo
                ref={videoRef}
                src="/video/avanzado/Actividad_6_scene_2.mp4"
            ...
            <div className="absolute inset-0 flex items-center justify-center z-20">
              <div className="flex flex-col items-center gap-6">
                <motion.div
                  ...
                  transition={{ duration: 0.8, ease: 'easeInOut' }}
                >
                  <JugarButton text="Continuar al menú" onClick={handleGoToHub} disabled={isAnimating} />
                </motion.div>
