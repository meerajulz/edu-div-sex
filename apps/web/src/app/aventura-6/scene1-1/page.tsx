...
import JuegoDosAventura6 from './JuegoDosAventura6/JuegoDosAventura6';

export default function Aventura6Scene1_1Page() {
  useActivityTracking();
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('av6-scene1-1-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('av6-scene1-1-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleGoToHub = async () => {
    ...
    setIsAnimating(true);
    playSound();
    await saveProgress('aventura-6', 'scene1-1', 'completed', 100, {
      video_watched: true,
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-b from-amber-200 via-yellow-100 to-orange-200 z-0" />
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          ...
          >
            <JugarButton text="LOS CELOS" onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
        <div className="absolute" style={containerStyle}>
          {!videoEnded ? (
            <>
              <OptimizedVideo
                ref={videoRef}
                src="/video/avanzado/Actividad_6_scene_1-1.mp4"
      ...
      {showCongratulations && (
        ...
        >
          <motion.div
            className="bg-gradient-to-br from-amber-300 via-yellow-400 to-orange-500 p-8 rounded-3xl shadow-2xl max-w-md mx-4 text-center"
          ...
          >
            ...
            <motion.button
              ...
              className="bg-white text-amber-600 font-bold py-3 px-6 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50"
