...
import { AVENTURA_6_CONFIG } from '../components/ActivityMenu/activityConfig';
...
export default function Aventura6NivelAvanzadoPage() {
  ...
  useEffect(() => {
    const hasSeenIntro = localStorage.getItem('aventura-6-intro-seen') === 'true';
    if (hasSeenIntro) {
      setVideoEnded(true);
      setShowSun(true);
  ...
  const handleVideoEnd = () => {
    cleanupAudio();
    stopBackgroundMusic('aventura-6-bg');
    localStorage.setItem('aventura-6-intro-seen', 'true');
    setVideoEnded(true);
    setShowSun(true);
    setTimeout(() => setShowArdilla(true), 100);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      {!videoEnded && (!canPlayVideo || needsInteraction) && (
        <>
          <div className="absolute inset-0 z-50 bg-gradient-to-b from-rose-300 via-pink-300 to-red-200" />
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center">
            ...
            >
              <JugarButton text="Relaciones de pareja" onClick={handleJugarClick} />
            </motion.div>
          </div>
        </>
      )}
      ...
      {isExiting && (
        <div className="fixed inset-0 bg-gradient-to-b from-rose-200 via-pink-200 to-red-300 z-0" />
      )}
