...
// const { data: session } = useSession();
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      console.log(`🍎 Activity5-Scene2 iPhone: Connecting video to Web Audio API...`);
      ...
      console.log(`🍎 Activity5-Scene2 iPhone: ✅ Video connected to Web Audio`);
    } catch (e) {
      console.error('🍎 Activity5-Scene2 iPhone: ❌ Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    const handleVolumeChange = (event: CustomEvent) => {
      ...
      console.log(`🎵 Activity5-Scene2: Received global volume change: ${volume}`);
      ...
      if (video && video.readyState > 0) {
        ...
        if (isIPhone) {
          ...
          if (window.videoGainNode) {
            ...
            console.log(`📱 Activity5-Scene2 iPhone: Applied volume ${volume} via Web Audio`);
          }
        } else {
          video.muted = false;
          video.volume = volume;
          console.log(`🖥️ Activity5-Scene2 Desktop/Android/iPad: Applied volume ${volume}`);
        }
      }
