...
import { ACTIVITY_5_CONFIG } from '../components/ActivityMenu/activityConfig';

export default function Actividad5Page() {
...
useEffect(() => {
  console.log('🔒 Actividad-5: Session status changed:', status);
  console.log('🔒 Actividad-5: Session data:', session);
}, [session, status]);
...
useEffect(() => {
  const handleGlobalVolumeChange = (e: CustomEvent) => {
    ...
    console.log(`🎵 Activity5: Received global volume change: ${volume} (iPhone: ${isIPhone}, iPad: ${isIPad})`);
    ...
    if (video && video.readyState > 0) {
      if (isIPhone) {
        ...
        if (sharedGainNode && sharedGainNode.gain) {
          ...
          console.log(`📱 Activity5 iPhone: Volume applied via shared gainNode: ${volume}`);
        }
      } else if (isIPad) {
        // iPad: Direct volume control
        video.volume = volume;
        video.muted = volume === 0;
        console.log(`🔲 Activity5 iPad: Applied volume ${volume} directly (muted: ${volume === 0})`);
      } else {
        // Desktop/Android: Direct video volume
        video.muted = false;
        video.volume = volume;
        console.log(`🖥️ Activity5 Desktop/Android: Applied volume ${volume} directly`);
      }
    }
  };
  ...
  useEffect(() => {
    const hasSeenIntro = localStorage.getItem('actividad-5-intro-seen') === 'true';
    if (hasSeenIntro) {
      setVideoEnded(true);
      setShowSun(true);
  ...
  const handleVideoEnd = () => {
    ...
    localStorage.setItem('actividad-5-intro-seen', 'true');
    setVideoEnded(true);
  ...
  const handleJugarClick = async () => {
    ...
    playBackgroundMusic('/audio/Softy.mp3', 0.4, 'actividad-5-bg');
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      {!videoEnded && (!canPlayVideo || needsInteraction) && (
        <>
          <div className="absolute inset-0 z-50 bg-gradient-to-b from-green-400 via-emerald-300 to-teal-300" />
          ...
          {!videoEnded && canPlayVideo ? (
            <OptimizedVideo
              ...
              onLoadedData={async () => {
                ...
                try {
                  await initAudio();
                  console.log('🍎 Activity5: Audio initialized');
                } catch (e) {
                  console.warn('Activity5: Audio init failed:', e);
                }
                ...
                if (isIPhone) {
                  ...
                  try {
                    ...
                    console.log(`🔲 Activity5 iPad: Audio context initialized for playGameAudio`);
                  } catch (error) {
                    console.error('🔲 Activity5 iPad: Error setting up audio context:', error);
                  }
                }
              }}
              onPlay={() => {
                ...
                if (isIPhone) {
                  ...
                  console.log(`📱 Activity5 iPhone: Video ready, using Web Audio volume control`);
                } else if (isIPad) {
                  video.volume = currentVolume;
                  video.muted = currentVolume === 0;
                  console.log(`🔲 Activity5 iPad: Video ready, direct volume: ${currentVolume} (muted: ${currentVolume === 0})`);
                } else {
                  video.muted = false;
                  video.volume = currentVolume;
                  console.log(`🖥️ Activity5 Desktop/Android: Video ready, volume: ${currentVolume}`);
                }
              }}
              lazyLoad={true}
              lowPowerMode={true}
      ...
      {showDebug && (
        <div className="fixed bottom-0 left-0 bg-black/70 text-white p-2 max-w-xs max-h-40 overflow-auto text-xs z-[1000]">
          <div className="font-bold mb-1">Actividad-5 Debug</div>
