...
export default function Aventura2NivelAvanzadoPage() {
  ...
  // Skip intro video on return visits
  useEffect(() => {
    const hasSeenIntro = localStorage.getItem('aventura-2-intro-seen') === 'true';
    if (hasSeenIntro) {
      setVideoEnded(true);
      setShowSun(true);
  ...
  const handleVideoEnd = () => {
    cleanupAudio();
    stopBackgroundMusic('aventura-2-bg');
    localStorage.setItem('aventura-2-intro-seen', 'true');
    setVideoEnded(true);
    setShowSun(true);
    setTimeout(() => setShowArdilla(true), 100);
  ...
  const handleJugarClick = () => {
    ...
    // No intro video for aventura-2 yet — skip directly to menu
    handleVideoEnd();
  };
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      {!videoEnded && (!canPlayVideo || needsInteraction) && (
        <>
          <div className="absolute inset-0 z-50 bg-gradient-to-b from-blue-500 via-teal-400 to-cyan-300" />
          ...
          {!videoEnded && canPlayVideo ? (
            <video
              ...
              onPlay={async () => {
                ...
                try { await initAudio(); } catch (e) { console.warn('Aventura-2: Audio init failed:', e); }
                if (isIPhone) {
                  try {
                    ...
                    if (!sharedAudioContext) {
                      ...
                      if (AudioContext) {
                        ...
                        if (!window.globalGainNode) {
                          ...
                          window.globalGainNode = gainNode;
                        }
                      }
                    }
                    if (sharedAudioContext) {
                    ...
                    console.error('Aventura-2 iPhone: AudioContext setup failed:', e);
                  }
                }
              }}
            />
