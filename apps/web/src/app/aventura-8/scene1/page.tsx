...
import JuegoUnoActividad6 from '../../actividad-6/scene1/JuegoUnoActividad6/JuegoUnoActividad6';
...
export default function Aventura8Scene1Page() {
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      ...
      console.error('Aventura8-Scene1: Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('a8-scene1-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('a8-scene1-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleCloseJuegoUno = () => setShowJuegoUno(false);
  ...
  const handleGoToMenu = async () => {
    ...
    await saveProgress('aventura-8', 'scene1', 'completed', 100, {
    ...
    if (!juegoUnoCompleted) {
      return (
        <div className="flex flex-col items-center gap-6">
          <JugarButton onClick={handleOpenJuegoUno} disabled={isAnimating} text="Jugar" />
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          <motion.div animate={isAnimating ? { scale: [1, 1.3, 1], rotate: [0, -360] } : {}} transition={{ duration: 0.8, ease: 'easeInOut' }}>
            <JugarButton text='ZONAS PRIVADAS' onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
              ...
              <OptimizedVideo
                ...
                onPlay={async () => {
                  ...
                  try {
                    ...
                    console.error('Aventura8-Scene1: Audio setup failed:', error);
                  }
                }}
                lazyLoad={true}
                lowPowerMode={true}
