...
import JuegoTresActividad6 from '../../actividad-6/scene3/JuegoTresActividad6/JuegoTresActividad6';
...
export default function Aventura8Scene3Page() {
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      ...
      console.error('Aventura8-Scene3: Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('a8-scene3-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('a8-scene3-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleOpenJuegoTres = () => { playSound(); setShowJuegoTres(true); };
  const handleCloseJuegoTres = () => setShowJuegoTres(false);
  ...
  const handleGoToMenu = async () => {
    ...
    await saveProgress('aventura-8', 'scene3', 'completed', 100, {
    ...
    if (!juegoTresCompleted) {
      return (
        <div className="flex flex-col items-center gap-6">
          <JugarButton onClick={handleOpenJuegoTres} disabled={isAnimating} text="Jugar" />
            ...
            <>
              <OptimizedVideo
                ...
                onPlay={async () => {
                  ...
                  } catch (error) { console.error('Aventura8-Scene3: Audio setup failed:', error); }
                }}
                lazyLoad={true}
                lowPowerMode={true}
                maxRetries={3}
