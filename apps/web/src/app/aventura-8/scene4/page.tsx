...
import JuegoCuatroActividad6 from '../../actividad-6/scene4/JuegoCuatroActividad6/JuegoCuatroActividad6';
import JuegoCincoActividad6 from '../../actividad-6/scene4/JuegoCincoActividad6/JuegoCincoActividad6';
...
export default function Aventura8Scene4Page() {
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      ...
      console.error('Aventura8-Scene4: Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('a8-scene4-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('a8-scene4-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleOpenJuegoCuatro = () => { playSound(); setShowJuegoCuatro(true); };
  const handleCloseJuegoCuatro = () => setShowJuegoCuatro(false);
  ...
  const handleOpenJuegoCinco = () => { playSound(); setShowJuegoCinco(true); };
  const handleCloseJuegoCinco = () => setShowJuegoCinco(false);
  ...
  const handleGoToMenu = async () => {
    ...
    await saveProgress('aventura-8', 'scene4', 'completed', 100, {
    ...
    if (!juegoCuatroCompleted) {
      return (
        <div className="flex flex-col items-center gap-6">
          <JugarButton onClick={handleOpenJuegoCuatro} disabled={isAnimating} text="Jugar" />
      ...
      return (
        <div className="flex flex-col items-center gap-6">
          <JugarButton onClick={handleOpenJuegoCinco} disabled={isAnimating} text="Jugar" />
          {hasWatchedVideo && <VolverAVerButton onClick={handleReplayVideo} />}
        </div>
      );
    }
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          <motion.div animate={isAnimating ? { scale: [1, 1.3, 1], rotate: [0, -360] } : {}} transition={{ duration: 0.8, ease: 'easeInOut' }}>
            <JugarButton text='APRENDO A PREGUNTAR Y A NO INSISTIR' onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
            ...
            <>
              <OptimizedVideo
                ...
                onPlay={async () => {
                  ...
                  } catch (error) { console.error('Aventura8-Scene4: Audio setup failed:', error); }
                }}
                lazyLoad={true}
                lowPowerMode={true}
                maxRetries={3}
