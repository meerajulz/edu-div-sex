...
import JuegoDosActividad6 from '../../actividad-6/scene2/JuegoDosActividad6/JuegoDosActividad6';
...
export default function Aventura8Scene2Page() {
  ...
  const connectVideoToWebAudio = (video: HTMLVideoElement, audioContext: AudioContext) => {
    try {
      ...
      console.error('Aventura8-Scene2: Web Audio connection failed:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('a8-scene2-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => {
    localStorage.setItem('a8-scene2-video-watched', 'true');
    setVideoEnded(true);
  ...
  const handleOpenJuegoDos = () => { playSound(); setShowJuegoDos(true); };
  const handleCloseJuegoDos = () => setShowJuegoDos(false);
  ...
  const handleGoToMenu = async () => {
    ...
    await saveProgress('aventura-8', 'scene2', 'completed', 100, {
    ...
    if (!juegoDosCompleted) {
      return (
        <div className="flex flex-col items-center gap-6">
          <JugarButton onClick={handleOpenJuegoDos} disabled={isAnimating} text="Jugar" />
            ...
            <>
              <OptimizedVideo
                ...
                onPlay={async () => {
                  ...
                  } catch (error) { console.error('Aventura8-Scene2: Audio setup failed:', error); }
                }}
                lazyLoad={true}
                lowPowerMode={true}
                maxRetries={3}
