...
// GET /api/admin/reports-by-teacher - Get detailed student reports grouped by teacher for this admin
export async function GET(_request: NextRequest) {
  try {
    ...
    const teachersReports: TeacherReport[] = await Promise.all(
    ...
    const totalStudents = teachersReports.reduce((sum, t) => sum + t.totalStudents, 0);
    const activeStudents = teachersReports.reduce((sum, t) => sum + t.activeStudents, 0);
    const activeTeachers = teachersReports.filter(t => t.is_active).length;

    const overallStats = {
      totalTeachers: teachersReports.length,
      activeTeachers,
      totalStudents,
      activeStudents,
      averageProgress: teachersReports.length > 0 && totalStudents > 0
        ? Math.round(
            teachersReports.reduce((sum, t) => sum + (t.averageProgress * t.totalStudents), 0) / totalStudents
          )
        : 0,
      supervisionLevelDistribution: {
        nivel1: teachersReports.reduce((sum, t) => sum + t.supervisionLevelDistribution.nivel1, 0),
        nivel2: teachersReports.reduce((sum, t) => sum + t.supervisionLevelDistribution.nivel2, 0),
        nivel3: teachersReports.reduce((sum, t) => sum + t.supervisionLevelDistribution.nivel3, 0)
      }
    };

    return NextResponse.json({
      teachers: teachersReports,
      overallStats
    });

  } catch (error) {
    console.error('Error fetching admin reports by teacher:', error);
