...
export default function Aventura3NivelAvanzadoPage() {
  ...
  useEffect(() => {
    const hasSeenIntro = localStorage.getItem('aventura-3-intro-seen') === 'true';
    if (hasSeenIntro) {
      setVideoEnded(true);
      setShowSun(true);
  ...
  const handleVideoEnd = () => {
    cleanupAudio();
    stopBackgroundMusic('aventura-3-bg');
    localStorage.setItem('aventura-3-intro-seen', 'true');
    setVideoEnded(true);
    setShowSun(true);
    setTimeout(() => setShowArdilla(true), 100);
  ...
  const handleJugarClick = () => {
    ...
    // No intro video for aventura-3 yet — skip directly to menu
    handleVideoEnd();
  };
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      {!videoEnded && (!canPlayVideo || needsInteraction) && (
        <>
          <div className="absolute inset-0 z-50 bg-gradient-to-b from-purple-500 via-pink-400 to-yellow-400" />
      ...
      {isExiting && (
        <div className="fixed inset-0 bg-gradient-to-b from-purple-400 via-pink-300 to-yellow-200 z-0" />
      )}
