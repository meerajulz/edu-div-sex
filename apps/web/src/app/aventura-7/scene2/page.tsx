...
import JuegoDosAventura7 from './JuegoDosAventura7/JuegoDosAventura7';

export default function Aventura7Scene2Page() {
  useActivityTracking();
  ...
  useEffect(() => {
    ...
    setHasWatchedVideo(!!localStorage.getItem('av7-scene2-video-watched'));
  }, []);
  ...
  const handleVideoEnd = () => { localStorage.setItem('av7-scene2-video-watched', 'true'); setVideoEnded(true); setHasWatchedVideo(true); };
  ...
  const handleGoToHub = async () => {
    ...
    await saveProgress('aventura-7', 'scene2', 'completed', 100, { video_watched: true, completed_at: new Date().toISOString() });
  ...
  return (
    <motion.div className="relative min-h-screen overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
      ...
      {!showVideo ? (
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          <motion.div animate={isAnimating ? { scale: [1, 1.3, 1], rotate: [0, -360] } : {}} transition={{ duration: 0.8, ease: 'easeInOut' }}>
            <JugarButton text="EL PRESERVATIVO NOS PROTEGE" onClick={handleJugarClick} disabled={isAnimating} />
          </motion.div>
        </div>
      ) : (
        <div className="absolute" style={containerStyle}>
          {!videoEnded ? (
            <>
              <OptimizedVideo ref={videoRef} src="/video/avanzado/Actividad_7_scene_2.mp4"
      ...
      <JuegoDosAventura7 isVisible={showGameModal} onClose={() => setShowGameModal(false)} onGameComplete={handleGameContinue} />
      ...
      {showCongratulations && (
        <motion.div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
          <motion.div className="bg-gradient-to-br from-sky-300 via-blue-400 to-cyan-500 p-8 rounded-3xl shadow-2xl max-w-md mx-4 text-center" initial={{ scale: 0.5, y: 50 }} animate={{ scale: 1, y: 0 }} transition={{ type: 'spring', damping: 15, stiffness: 300 }}>
            ...
            <motion.button onClick={handleGoToHub} disabled={isAnimating} className="bg-white text-sky-600 font-bold py-3 px-6 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              Continuar al menú
            </motion.button>
          </motion.div>
        </motion.div>
